...
public abstract class GameModel {
   ...
   */
  public GameModel(String name, int boardSize, GameType gameType) {
    this.name = name;
    this.board =
        gameType == GameType.TICTACTOE
            ? new TicTacToeBoard(boardSize)
            : new OthelloBoard(boardSize);
  }
  ...
  /**
   * check if the given player has won by checking if the GameState value with index users id + 1 is
