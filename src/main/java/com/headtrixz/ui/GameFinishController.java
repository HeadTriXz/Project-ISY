...
public class GameFinishController {
  ...
  /** FXML init method. Gets called when the screen has loaded. Shows the board with who has won. */
  public void initialize() {
    String opponentName = game.getPlayer(1).getUsername();
    String text =
        switch (game.getState()) {
          case PLAYER_ONE_WON -> String.format(
              "Gefeliciteerd, je hebt van %s gewonnen.", opponentName);
